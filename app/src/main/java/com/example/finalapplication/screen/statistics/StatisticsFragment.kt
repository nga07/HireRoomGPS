...
class StatisticsFragment :
    ...
    override fun bindData() {
        viewModel.apply {
            roomHired.observe(this@StatisticsFragment) { data ->
                var number: Int = data.size
                numberAllHired = number.toFloat()
                Log.d("numberHired", "bindData1: $numberAllHired")
                roomAllHires.clear()
                roomAllHires.addAll(data)
            }
            roomNotHired.observe(this@StatisticsFragment) { data ->
                var number: Int = data.size
                numberAllNotHired = number.toFloat()
                Log.d("numberHired", "bindData2: $numberAllNotHired")
            }
            roomNotHired2.observe(this@StatisticsFragment) { data ->
                var number: Int = data.size
                numberAllNotHired2 = number.toFloat()
                Log.d("numberHired", "bindData2: $numberAllNotHired2")
            }
            roomVerifyHired.observe(this@StatisticsFragment) { data ->
                var number: Int = data.size
                numberVerifiedHired = number.toFloat()
                Log.d("numberHired", "verifiedData1: $numberVerifiedHired")
                roomVerifiedHire.clear()
                roomVerifiedHire.addAll(data)
            }
            roomVerifyNotHired.observe(this@StatisticsFragment) { data ->
                var number: Int = data.size
                numberVerifiedNotHired = number.toFloat()
                Log.d("numberHired1", "verifiedData2: $numberVerifiedNotHired")
            }
            roomVerifyNotHired2.observe(this@StatisticsFragment) { data ->
                var number: Int = data.size
                numberVerifiedNotHired2 = number.toFloat()
                Log.d("numberHired1", "verifiedData3: $numberVerifiedNotHired")
            }
        }
    }

    fun countHired(start: Long, end: Long) {
        var cnt = 0
        copyTimeRoom.clear()
        timeRoom.clear()
        countRoom.clear()
    ...
    override fun initData() {
        ...
        lifecycleScope.launch {
            delay(TimeConstant.DELAY_DATA)
            countHired(0, System.currentTimeMillis())
...
//        rooms.add(PieEntry(numberAllNotHired, "Chưa Thuê"))
    ...
    fun barChartRevenue() {
        ...
        var barDataSet = BarDataSet(visit, "Chi phí")
        var partColors = ArrayList<Int>()
        partColors.add(Color.BLUE)
        barDataSet.setColors(partColors)
        var textColors = ArrayList<Int>()
        ...
        binding.barChart3.description.text = "Chi phí"
