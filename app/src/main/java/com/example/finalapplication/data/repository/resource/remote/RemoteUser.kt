...
class RemoteUser : UserDataSource.Remote {
    ...
    override suspend fun getUserByEmail(email: String, listen: Listenner<User?>) {
        database.collection(User.users)
            .whereEqualTo("userAccount.email",email)
            .get()
            .addOnCompleteListener { rs ->
                if(rs.isSuccessful){
                    if(rs.result.documents.isEmpty()){
                        listen.onSuccess(null)
                        return@addOnCompleteListener
                    }
                    val user = rs.result.documents.first().toObject(User::class.java)
                    if(user!=null) listen.onSuccess(user)
                } else listen.onError(rs.exception.toString())
            }
    }
